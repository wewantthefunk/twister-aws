...
- **In scope today:** **Secrets Manager** and **SSM** (`X-Amz-Target`, JSON 1.1), **S3** path-style REST (scope `s3`: buckets + objects under `s3DataPath/{region}/…`), **SQS** (scope `sqs`), **Lambda**-compatible control/invoke (scope `lambda`, `X-Amz-Target` `Lambda_20150331.*`, Docker; see **`docs/LAMBDA.md`**), **EC2** Query API subset (scope `ec2`, form + XML, Docker-backed `RunInstances`; **`internal/ec2`**, AMI catalog JSON), **IAM** `CreateAccessKey` (form, XML), shared **SigV4**, file-backed data (`dataPath` / env).
...
| **Auth** | `credentials.Provider` + CSV allowlist; `sigv4.Verify` uses the **signing service** in the credential scope (`secretsmanager`, `ssm`, `s3`, `sqs`, `lambda`, `iam`, `ec2`, …). For JSON 1.1, the scope must **match** the `X-Amz-Target` service prefix (after canonicalization). |
| **HTTP routing** | `internal/awsserver.Router` — read body, verify SigV4, then by scope: **IAM** / **EC2** / **SQS** form handlers vs **JSON 1.1** `X-Amz-Target` to `awsserver.Service` implementations. |
| **Service modules** | One package per “AWS product” surface (e.g. `internal/secretsmanager`, `internal/ssm`, `internal/iam`, `internal/ec2`); implement `awsserver.Service` for JSON 1.1 products; IAM/EC2/SQS Query stay separate from JSON 1.1. |
...
| **Refresh** | `GET/POST /refresh` — reload credentials, secret store, parameter store, and **EC2** `state.json` from disk; extend `awsserver.Refresher` when new file-backed state exists. |
...
**Adding a new IAM / EC2 / SQS Query action:** extend the relevant package (`internal/iam`, `internal/ec2`, `internal/sqs`), align XML/errors with AWS docs where reasonable, add tests, update `README.md`.
